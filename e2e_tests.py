...
class E2eTests(unittest.TestCase):
    ...
    def test_response_time(self):
        ...
        # response = machine_manager_api.get_machines(self.machine_manager_url)
        response = subprocess.run("curl -X GET -o /dev/null 127.0.0.1:5000 -w %{time_total}".split(), capture_output=True)

        print("result:", str(response.stdout))
